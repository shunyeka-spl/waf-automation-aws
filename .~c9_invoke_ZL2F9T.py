...
def query():
    ...
    QUERY = '''SELECT c_ip, cs_host, cs_uri_stem, x_host_header, COUNT(c_ip) AS Frequency FROM "CloudFrontLogsTimeSeriesDb-xBECBoapfI2U"."RealtimeLogsTable-7MYNnsSemTCE"  WHERE time between ago(150m) and now() GROUP BY c_ip,cs_host,cs_uri_stem,x_host_header ORDER BY Frequency DESC '''
